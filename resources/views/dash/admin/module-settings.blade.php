    ...
    <div class="admin-card space-y-4">
        ...
        @if($moduleKey === 'smtp_general' || $moduleKey === 'smtp_transactional')
            <form action="{{ route($moduleKey === 'smtp_general' ? 'dash.admin.smtp.general.save' : 'dash.admin.smtp.transactional.save', ['authkey' => $authkey]) }}" method="POST" class="space-y-3">
                ...
                <div class="grid gap-3 md:grid-cols-2">
                    <input name="host" value="{{ old('host', $moduleSettings['host'] ?? '') }}" placeholder="Host" class="admin-input">
                    <input name="port" value="{{ old('port', $moduleSettings['port'] ?? '') }}" placeholder="Port" class="admin-input">
                    <input name="username" value="{{ old('username', $moduleSettings['username'] ?? '') }}" placeholder="Username" class="admin-input">
                    <input name="password" value="{{ old('password', $moduleSettings['password'] ?? '') }}" placeholder="Password" class="admin-input">
                    ...
                    <input name="sender_email" value="{{ old('sender_email', $moduleSettings['sender_email'] ?? '') }}" placeholder="Sender Email" class="admin-input">
                    <input name="sender_name" value="{{ old('sender_name', $moduleSettings['sender_name'] ?? '') }}" placeholder="Sender Name" class="admin-input">
                </div>
        ...
        @elseif($moduleKey === 'sms')
            <form action="{{ route('dash.admin.sms.config.save', ['authkey' => $authkey]) }}" method="POST" class="space-y-3">
                @csrf
                <input name="endpoint" value="{{ old('endpoint', $moduleSettings['endpoint'] ?? 'https://console.melipayamak.com/api/send/otp/{token}') }}" placeholder="API Endpoint" class="admin-input w-full">
                <input name="api_token" value="{{ old('api_token', $moduleSettings['api_token'] ?? '') }}" placeholder="API Token" class="admin-input w-full">
                <input name="sender_number" value="{{ old('sender_number', $moduleSettings['sender_number'] ?? '') }}" placeholder="Sender Number" class="admin-input w-full">
                <div class="admin-actions"><button class="admin-btn" type="submit"><span class="material-icons">save</span>ذخیره</button></div>
            </form>
        ...
        @elseif($moduleKey === 'contact')
            <form action="{{ route('dash.admin.contact.page.info.save', ['authkey' => $authkey]) }}" method="POST" class="space-y-3">
                @csrf
                <input name="title" value="{{ old('title', $moduleSettings['title'] ?? 'اطلاعات تماس') }}" placeholder="عنوان" class="admin-input w-full">
                <textarea name="description" rows="4" placeholder="توضیحات" class="admin-input w-full">{{ old('description', $moduleSettings['description'] ?? '') }}</textarea>
                <div class="grid gap-3 md:grid-cols-2">
                    <input name="phone" value="{{ old('phone', $moduleSettings['phone'] ?? '') }}" placeholder="Phone" class="admin-input">
                    <input name="email" value="{{ old('email', $moduleSettings['email'] ?? '') }}" placeholder="Email" class="admin-input">
                    <input name="address" value="{{ old('address', $moduleSettings['address'] ?? '') }}" placeholder="Address" class="admin-input md:col-span-2">
                    <input name="work_hours" value="{{ old('work_hours', $moduleSettings['work_hours'] ?? '') }}" placeholder="Work hours" class="admin-input md:col-span-2">
                </div>
                <div class="admin-actions"><button class="admin-btn" type="submit"><span class="material-icons">save</span>ذخیره</button></div>
            </form>
        @elseif($moduleKey === 'affiliate')
            <form action="{{ route('dash.admin.affiliate.settings.save', ['authkey' => $authkey]) }}" method="POST" class="space-y-3">
                @csrf
                <div class="grid gap-3 md:grid-cols-2">
                    <input name="merchant_id" value="{{ old('merchant_id', $moduleSettings['merchant_id'] ?? '') }}" placeholder="Merchant ID" class="admin-input">
                    <input name="access_token" value="{{ old('access_token', $moduleSettings['access_token'] ?? '') }}" placeholder="Access Token" class="admin-input">
                    <input name="url_prefix" value="{{ old('url_prefix', $moduleSettings['url_prefix'] ?? 'https://a.bslm.ir/api/v1/tracking/click/') }}" placeholder="URL Prefix" class="admin-input md:col-span-2">
                    <input name="cache_ttl_minutes" value="{{ old('cache_ttl_minutes', $moduleSettings['cache_ttl_minutes'] ?? 120) }}" placeholder="Cache TTL" class="admin-input">
                </div>
                <div class="admin-actions"><button class="admin-btn" type="submit"><span class="material-icons">save</span>ذخیره</button></div>
            </form>
