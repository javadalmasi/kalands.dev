<x-layouts.admin-dashboard title="مدیریت کاربران">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <h1 class="admin-page-title !mb-0">مدیریت کاربران</h1>
    ...
    <form class="admin-card mb-6 grid gap-3 md:grid-cols-4" method="GET">
        <div class="space-y-1">
            <label class="text-[10px] font-bold opacity-60 px-1">جستجو (نام، ایمیل، موبایل)</label>
            <input type="text" name="q" value="{{ $q }}" placeholder="جستجو..." class="w-full rounded-lg border border-slate p-2 text-sm dark:bg-slate-800 dark:text-white dark:border-white/10 dark:focus:bg-slate-700">
        </div>
        <div class="space-y-1">
    ...
    <form action="{{ route('dash.admin.users.bulk', ['authkey' => $authkey]) }}" method="POST" id="bulk-action-form">
        @csrf
        <div id="bulk-toolbar" class="admin-card mb-4 flex flex-wrap items-center justify-between gap-4 bg-primary/5 border-primary/20">
            <div class="flex items-center gap-4">
                <label class="inline-flex items-center gap-2 text-xs cursor-pointer bg-slate/5 px-3 py-2 rounded-lg border border-slate/10 hover:bg-slate/10 transition-colors">
                    <input type="checkbox" id="select-all-users" class="rounded border-slate/30">
            ...
            <div class="text-[10px] font-bold opacity-40">
                <span id="selected-count" class="hidden">0 مورد انتخاب شده | </span>
